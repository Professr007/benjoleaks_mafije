...
MySQL.ready(function()
        MySQL.Async.fetchAll("SELECT * FROM jobs WHERE 1",{},
            function(name)
                for _, v in ipairs(name) do
                    ...
                    MySQL.Async.fetchAll("SELECT * FROM job_grades WHERE job_name = @job",{["@job"] = v.name},
                        function(gradeInfo)
...
ESX.RegisterServerCallback("core_jobutilities:getBossMenuData",function(source, cb, job)
        ...
        local employees = {}
        MySQL.Async.fetchAll("SELECT * FROM users WHERE job = @job",{["@job"] = job},
            function(info)
                for _, v in ipairs(info) do
                    if v.job_grade == nil then
                        v.job_grade = 0 end
                    table.insert(employees,{identifier = v.identifier,fullname = (v.firstname .. ' ' .. v.lastname) or "",grade = v.job_grade,grade_label = JobInfo[job].grades[tonumber(v.job_grade)].label})
                end
...
AddEventHandler("core_jobutilities:hire",function(id, job)
        ...
        if xPlayer then
            ...
            xPlayer.setJob(job, 0)
             ...
             MySQL.Async.execute("UPDATE users SET `job`=@job, `job_grade`=@rank WHERE `identifier` = @identifier",{["@job"] = job, ["@rank"] = "0", ["@identifier"] = xPlayer.identifier},
                function()
                end)
        else
            TriggerClientEvent('okokNotify:Alert', source, "", "Radnja uspesno izvrsena", 5000, 'success')
...
AddEventHandler("core_jobutilities:fire",function(identifier, job)
        ...
        if xPlayer then
            ...
            MySQL.Async.execute("UPDATE users SET `job`=@job, `job_grade`=@rank WHERE `identifier` = @identifier",{["@job"] = "unemployed", ["@rank"] = "0", ["@identifier"] = identifier},
                function()
                end)
    end)
...
AddEventHandler("core_jobutilities:givebonus",function(identifier, amount, job)
        ...
        if xPlayer then
               ...
               TriggerClientEvent('okokNotify:Alert', xPlayer.source, "", "Primili ste bonus", 5000, 'success')
                 ...
                 MySQL.Async.execute("UPDATE users SET `bank`=`bank` + @amount WHERE `identifier` = @identifier",{["@amount"] = tonumber(amount), ["@identifier"] = identifier},
                function()
                    TriggerClientEvent('okokNotify:Alert', source, "", "Dali ste bonus", 5000, 'success')
                end)
        end
...
AddEventHandler("core_jobutilities:withdraw",function(job, amount)
        local src = source
        local xPlayer = ESX.GetPlayerFromId(src)
       TriggerEvent('esx_addonaccount:getSharedAccount', 'society_' .. job, function(account)
                if true then
...
AddEventHandler("core_jobutilities:setRank",function(identifier, job, rank)
        local src = source
        local xPlayer = ESX.GetPlayerFromIdentifier(identifier)
        if xPlayer then
            if xPlayer.getJob().name == job then
            ...
            MySQL.Async.execute("UPDATE users SET `job`=@job, `job_grade`=@rank WHERE `identifier` = @identifier",{["@job"] = job, ["@rank"] = rank, ["@identifier"] = identifier},
                function()
                end)
    end)
